...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        bottom_bar --set $NAME icon.color=0xffeed49f label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        bottom_bar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes icon.color=0xffa6da95

    elif [ $PLAYER_STATE = "Paused" ]; then
        bottom_bar --set $NAME icon.color=0xffeed49f
